...
#if defined(ZSIGN_IOS)
static bool RemoveFolderRecursive(const string& strFolder)
{
...
#elif defined(ZSIGN_IOS)
	return RemoveFolderRecursive(string(szFolder));
#else
...
#else
	#if defined(ZSIGN_IOS)
		// realpath may not work reliably in iOS sandbox; just use the path as-is
		char path[PATH_MAX] = { 0 };
		strncpy(path, szPath, PATH_MAX - 1);
		strPath = path;
#else
		char path[PATH_MAX] = { 0 };
